...
export default function VenueMenu() {
  ...
  const MENU_DATA = useMemo(() => {
    ...
    const venueObj = collegeObj?.venues.find(
      (v) => v.name.toLowerCase() === venueParam,
    );
  ...
  return (
    <div className="w-full min-h-screen bg-[#FFD54F] font-sans">
      ...
      <header className="py-4 px-6 flex items-center justify-between sticky top-0 bg-[#FFD54F] z-30">
        <button
          onClick={() => router.back()}
          className="bg-white p-3 text-gray-500 rounded-2xl shadow-lg"
        >
          <ChevronLeft size={24} />
        </button>
        <div className="text-center">
          ...
          <p className="text-[14px] font-bold text-red-600 tracking-widest uppercase">
            Menu Selection
          </p>
        </div>
        <div className="w-12" />
            ...
            {filteredItems.map((item) => (
              ...
              >
                <img
                  src={item.img}
                  alt={item.name}
                  className="w-16 h-16 md:w-24 md:h-24 rounded-2xl object-cover shadow-md"
                />
                <div className="flex-1">
                  ...
                  <h3 className="font-black text-gray-900 leading-tight uppercase text-sm mb-1">
                    {item.name}
                  </h3>
                ...
                <div className="flex flex-row items-center gap-2 md:flex-col bg-white p-1 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <span
                    className={`font-black ${cart[item.id] ? "text-black" : "text-gray-200"} text-sm w-6 text-center`}
                  >
                    {cart[item.id] || 0}
                  </span>
